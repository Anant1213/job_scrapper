...
def _seniority_penalty(title: str):
    t = (title or "").lower()
    if any(x in t for x in ["manager","lead","vp","director","principal","head"]): return 8
    if "intern" in t or "trainee" in t: return 12
    return 0
...
def score_job(job):
    ...
    min_exp = job.get("min_exp"); max_exp = job.get("max_exp")
